...
@javascript
  ...
  RSpec.feature  %{navigation}  do
    ...
    scenario %{as I head over the head links in navigation
    the sublinks extend as appropriate}  do

        link_1 = find %{div a},:text => %{MODULES}
        link_1.hover
        sublink_0 = find %{div a},:text => %{BASIC COURSES}
        sublink_1 = find %{div a},:text => %{GUIDELINES}
        sublink_2 = find %{div a},:text => %{ADVANCED COURSES}
        expect(sublink_0.visible?).to be true
        expect(sublink_1.visible?).to be true
        expect(sublink_2.visible?).to be true

        link_0 = find %{div a},:text => %{ABOUT}
        link_0.hover
        sublink_0 = find %{div a},:text => %{APPRECIATION}
        sublink_1 = find %{div a},:text => %{OUR TEAM}
        expect(sublink_0.visible?).to be true
        expect(sublink_1.visible?).to be true
    ...
    scenario %{when I head to the home page im at the home page}  do
        visit %{/videos}
        home_link = find %{div a},:text => %{HOME}
        home_link.click
        title = find %{h1},:text => %{Welcome to Law For Learners}
        expect(title.text).to be_truthy
        visit %{/videos}
        logo_img = first %{.a_p_p_NavHeaderImg}
        logo_img.click
        title = find %{h1},:text => %{Welcome to Law For Learners}
        expect(title.text).to be_truthy
    end

    scenario %{when I head to the videos page im at the videos page}  do
        visit %{/home}
        videos_link = find %{div a},:text => %{VIDEOS}
        videos_link.click
        title = find %{h1},:text => %{Videos}
        expect(title.text).to be_truthy
