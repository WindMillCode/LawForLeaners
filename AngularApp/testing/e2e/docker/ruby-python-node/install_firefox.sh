...
if uname -a | grep Darwin > /dev/null 2>&1; then
  ...
  $SUDO rm -f "$FIREFOX_FILE_NAME.$FILE_EXT"
  $SUDO mv firefox "/usr/local/bin/firefox-$FIREFOX_VERSION"
  $SUDO chmod +x "/usr/local/bin/firefox-$FIREFOX_VERSION/firefox"
  $SUDO ln -s "/usr/local/bin/firefox-$FIREFOX_VERSION/firefox" /usr/local/bin/firefox
