...
if uname -a | grep Darwin > /dev/null 2>&1; then
  ...
  tar -xjf "$FIREFOX_FILE_NAME.$FILE_EXT"
  rm -f "$FIREFOX_FILE_NAME.$FILE_EXT"
  mv firefox "/usr/local/bin/firefox-$FIREFOX_VERSION"
  chmod +x "/usr/local/bin/firefox-$FIREFOX_VERSION/firefox"
  ln -s "/usr/local/bin/firefox-$FIREFOX_VERSION/firefox" /usr/local/bin/firefox
